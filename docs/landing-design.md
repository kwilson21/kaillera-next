...
**Re-checked at build start (2026-09-26, M0):** `CF_TURN_KEY_ID` and
`CF_TURN_API_TOKEN` **are set** on `srv-dar79re0tbcc7399r1mg`. Evidence: the
same three `201 Created` credential calls in the service logs; the server
only makes that call when both variables are present (`turn.configured()`).
The Render API available to the build session doesn't list env var names,
so this is log evidence, not a dashboard read; the owner can confirm under
service → Environment. Still no Key Value instance in the workspace. The connection-failure copy (§7.4) stays as the
