...
| `_rbShutdownHold` (Smash Remix GAMEPLAY→MENU: C-rollback shutdown waits until the live state is confirmed — no replay pending, every consumed remote input present and fed to C) | `RB_SHUTDOWN_HOLD_MS = 3000` | Shut down anyway; the peers may now differ | `RB-SHUTDOWN-HOLD-TIMEOUT` | Remix pause follow-up to PR #25 |
