...
| Reviewer + scope/safety gate + spec writer + determinism gatekeeper + creative reframe | **Claude Opus** | Synthesis quality and calibration are the deliverable. Reviews PM backlog for scope/safety/feasibility-against-determinism; reviews UX recommendations for design-system consistency and accessibility. **Beyond approve/reject:** when Claude rejects a PM or UX item, the review pass also produces a *creative reframe* — "this item is wrong as scoped, but here's a different way to think about the underlying problem." This is the value of routing the high-creativity model through review rather than only through safety: the creative differentiator shows up where it would otherwise be wasted. |
...
- **PM weekly outputs** get a Claude scope/safety pass before any backlog item is filed for engineering. Claude rejects items that conflict with determinism / R1-R6 / known scope discipline, and for rejected items, includes a creative-reframe paragraph (what's a different way to frame the underlying problem). Codex reviews any backlog item that proposes a technical change for feasibility before it becomes a brief.
