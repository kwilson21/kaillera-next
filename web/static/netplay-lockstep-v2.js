/**
 * kaillera-next — Prototype C v2: Lockstep via pauseMainLoop/resumeMainLoop
 *
 * True Kaillera-style dual-emulator lockstep. Both peers run their own
 * EmulatorJS instance and stay in sync by exchanging inputs each frame.
 *
 * Frame stepping approach (v2):
 *   Instead of intercepting requestAnimationFrame (v1 — fragile across
 *   browser tabs), we use Emscripten's pauseMainLoop/resumeMainLoop.
 *   The emulator is paused. Each tick: apply inputs → resumeMainLoop →
 *   emulator runs 1 frame via its native rAF → detect frame advance →
 *   pauseMainLoop. This keeps the runner in its natural rAF context
 *   (no Wasm audio OOB crashes, proper WebGL compositing).
 *
 *   Playwright verified: 300/300 frames, all delta=1, zero crashes.
 *
...
(function () {
  ...
  let _manualMode        = false;  // true when emulator is paused for lockstep
  let _tickTimer         = null;   // setInterval ID for the tick loop
  let _stepping          = false;  // true while waiting for a frame to complete
  ...
  function createPeer(remoteSid, remoteSlot, isInitiator) {
    ...
    peer.pc.onconnectionstatechange = () => {
      ...
      if (s === 'failed' || s === 'disconnected') {
        console.log('[lockstep-v2] peer', remoteSid, 'connection', s);
  ...
  function setupDataChannel(remoteSid, ch) {
    ...
    ch.onopen = () => {
      const peer = _peers[remoteSid];
      if (!peer) return;
      console.log('[lockstep-v2] DC open with', remoteSid, 'slot:', peer.slot);
      peer.ready = true;
      ch.send('ready');
    ...
    ch.onclose = () => {
      console.log('[lockstep-v2] DC closed with', remoteSid);
      handlePeerDisconnect(remoteSid);
    };

    ch.onerror = (e) => console.log('[lockstep-v2] DC error:', remoteSid, e);
  ...
  function handlePeerDisconnect(remoteSid) {
    ...
    window._peers = _peers;
    console.log('[lockstep-v2] peer disconnected:', remoteSid);
    if (_running) {
  ...
  function startGameSequence() {
    ...
    const waitForEmu = () => {
      ...
      console.log('[lockstep-v2] emulator booted (' + frames + ' frames)');
      _selfEmuReady = true;
  ...
  function checkBothEmuReady() {
    ...
    console.log('[lockstep-v2] both emulators ready — syncing initial state');
    setStatus('Syncing…');
  ...
  function checkBothLockstepReady() {
    ...
    if (_running) return;
    console.log('[lockstep-v2] both sides lockstep-ready — GO');
  ...
  async function sendInitialState() {
    ...
    try {
      ...
      console.log('[lockstep-v2] failed to send initial state:', err);
    }
  }

  async function handleSaveState(msg) {
    console.log('[lockstep-v2] received initial state');
    ...
    try {
      ...
      console.log('[lockstep-v2] initial state decompressed (' + bytes.length + ' bytes)');
      ...
      console.log('[lockstep-v2] failed to decompress initial state:', err);
    }
  }

  // ── Manual frame stepping ─────────────────────────────────────────────
  //
  // Strategy: use Emscripten's pauseMainLoop/resumeMainLoop to gate
  // frame advance. The emulator stays paused. Each tick:
  //   1. Apply inputs via simulateInput
  //   2. resumeMainLoop() — emulator schedules a native rAF
  //   3. Native rAF fires → retro_run executes → frame advances
  //   4. We detect the advance (frame count incremented) → pauseMainLoop()
  //
  // The tick loop runs via setInterval(16) (~60fps). On each interval:
  //   - If we're waiting for a frame to complete (_stepping), check if done
  //   - If not stepping and inputs are ready, start the next frame step
  //   - If inputs aren't ready, wait (stall)
  //
  // No rAF interception. No calling the runner from setTimeout. The runner
  // always executes in its natural rAF context — proper audio + GL compositing.
  ...
  function enterManualMode() {
    ...
    _manualMode = true;
    _stepping = false;
    console.log('[lockstep-v2] manual mode active (paused)');
  }
  ...
  function exitManualMode() {
    ...
    const mod = window.EJS_emulator.gameManager.Module;
    mod.resumeMainLoop();
    console.log('[lockstep-v2] manual mode exited — normal playback');
  }

  // ── Lockstep tick loop ────────────────────────────────────────────────

  let _stepFrameBefore = 0;  // frame count before current step
  ...
  function startLockstep() {
    ...
    console.log('[lockstep-v2] lockstep started — slot:', _playerSlot);
    setStatus('Connected — game on!');

    // Use setInterval for the tick loop. This fires regardless of
    // whether the tab is in the foreground or background (unlike rAF).
    // The actual frame rate is gated by resumeMainLoop → native rAF,
    // so we won't exceed 60fps even though the interval is 4ms.
    _tickTimer = setInterval(tick, 4);
  }
  ...
  function tick() {
    ...
    // If we're waiting for a frame step to complete, check if done
    if (_stepping) {
      const now = mod._get_current_frame_count();
      if (now > _stepFrameBefore) {
        // Frame advanced! Pause immediately.
        mod.pauseMainLoop();
        _stepping = false;
      ...
      // Still waiting for rAF to fire — do nothing, check again next interval
      return;
    }

    // Not stepping — try to start the next frame
    ...
    // Step 2: Check if we have all inputs needed
    const applyFrame = _frameNum - DELAY_FRAMES;

    if (applyFrame >= 0) {
      if (!(applyFrame in _remoteInputs)) {
        // STALL: wait for remote input — do nothing, check again next interval
        if (_stallStart === 0) _stallStart = performance.now();
        const waited = performance.now() - _stallStart;
        if (waited >= MAX_STALL_MS) {
          console.log('[lockstep-v2] stall timeout at frame', applyFrame, '(' + Math.round(waited) + 'ms)');
    ...
    // Step 4: Start frame advance — resume the emulator's native rAF loop
    _stepFrameBefore = mod._get_current_frame_count();
    _stepping = true;
    mod.resumeMainLoop();
    // The emulator will run 1 frame via its native rAF. The next tick()
    // interval will detect the frame count increment and pause again.
  }
  ...
  function handleStateHash(msg) {
    ...
    if (msg.hash === _lastHash) {
      if (_desynced) {
        console.log('[lockstep-v2] hashes match post-resync at frame', msg.frame);
        _desynced = false;
        window._desynced = false;
        setStatus('Connected — game on!');
      }
    ...
    console.log('[lockstep-v2] DESYNC at frame', msg.frame,
  ...
  function handleResyncRequest() {
    ...
    console.log('[lockstep-v2] received resync request');
    _desynced = true;
    window._desynced = true;
    sendResyncState();
  }
  ...
  async function sendResyncState() {
    ...
    try {
      ...
      const compressed = await compressState(bytes);
      const b64 = uint8ToBase64(compressed);
      console.log('[lockstep-v2] sending resync state');
      socket.emit('data-message', {
        type: 'resync-state',
        frame: _frameNum,
        data: b64,
      ...
      console.log('[lockstep-v2] resync send failed:', err);
      _desynced = false;
      window._desynced = false;
    }
  }
  ...
  async function handleResyncState(msg) {
    console.log('[lockstep-v2] received resync state for frame', msg.frame);
    try {
      const compressed = base64ToUint8(msg.data);
      const bytes = await decompressState(compressed);
      applyResync(msg.frame, bytes);
    } catch (err) {
      console.log('[lockstep-v2] resync load failed:', err);
      _desynced = false;
      window._desynced = false;
    }
  }
  ...
  function applyResync(frame, stateBytes) {
    ...
    try {
      ...
      console.log('[lockstep-v2] loadState failed:', err);
      _desynced = false;
      window._desynced = false;
      return;
    }
    ...
    _lastHashFrame = -1;
    _stallStart    = 0;
    console.log('[lockstep-v2] resync applied at frame', frame);
    setStatus('Resynced');
  }
  ...
  function applyStandardCheats() {
    const attempt = () => {
      ...
      try {
        ...
        console.log('[lockstep-v2] applied', SSB64_ONLINE_CHEATS.length, 'standard cheats');
